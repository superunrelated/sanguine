...
module.exports = sanguine = (function() {
  function sanguine() {
    ...
    this._parseConfig = __bind(this._parseConfig, this);    this.base = null;
    this.images = [];
    this.filecount = 0;
    this.existCount = 0;
    this.cleanup = [];
  ...
  sanguine.prototype._parseDirectory = function(source, target, set) {
    ...
    return _.each(files, function(file, key) {
      ...
      if (stats.isDirectory()) {
        ...
        if (colors) {
          _this._addFiles(colors, TYPE_COLOR, src, target, set.appendQuality);
        }
        ...
        if (jpgs) {
          return _this._addFiles(jpgs, TYPE_JPG, src, target, set.appendQuality);
        }
      }
    });
  };

  sanguine.prototype._addFiles = function(versions, type, src, target, appendQuality) {
    var _this = this;
    ...
    if (versions) {
      ...
      return _.each(versions, function(quality) {
        ...
        if (_this.retinaRegexp.test(path.basename(src))) {
          tgt = path.join(path.dirname(tgt), path.basename(tgt).replace(_this.retinaTagsRegexp, '$11x'));
  ...
  sanguine.prototype._generateComplete = function() {
    ...
    if (this.report) {
      ...
      log(this.images(JSON.stringify(this.images, null, 2)));
