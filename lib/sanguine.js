var CONFIG, easyimg, exec, fs, log, path, sanguine, trace, _,
...
module.exports = sanguine = (function() {
  ...
  function sanguine() {
    ...
    this.unretinaRegexp = /-2x/g;
  }

  sanguine.prototype.optimize = function(configpath) {
    var _this = this;
    return this._loadConfig(configpath, function(err) {
      return _this._parse();
    });
  };

  sanguine.prototype._loadConfig = function(configpath, fn) {
    var _this = this;
    if (configpath.indexOf(CONFIG) === -1) {
      configpath += CONFIG;
    }
    configpath = path.normalize(configpath);
    return fs.readFile(configpath, 'utf8', function(err, data) {
      if (err) {
      ...
      _this.base = path.dirname(configpath);
      return fn();
    });
  };
        ...
        return fs.stat(src, function(err, stats) {
          ...
          if (stats.isDirectory()) {
            ...
            return fs.exists(target, function(exists) {
              ...
              return fs.mkdir(target, null, function(err) {
                if (err) {
                  throw err;
                }
                return _this._parseFile(src, target, set);
  ...
  sanguine.prototype._parseFile = function(src, target, set) {
    var fileColors, fileJpg, fileTarget, unretinaTarget,
      _this = this;
    if (this.unretinaRegexp.test(src)) {
      unretinaTarget = src.replace(this.unretinaRegexp, '-1x');
    ...
    if (!(fileColors != null) && !(fileJpg != null)) {
      fileColors = set.colors;
      fileJpg = set.jpg;
    }
    this.filecount += fileColors.length + fileJpg.length;
    _.each(fileColors, function(color) {
      return _this._optimizeFile(src, fileTarget, color, _this._fileParsed);
    });
    return _.each(fileJpg, function(quality) {
      return _this._jpgFile(src, fileTarget, quality, _this._fileParsed);
    });
  };

  sanguine.prototype._fileParsed = function(err, file) {
    if (err) {
      return log(err);
    }
    this.filecount--;
  ...
  sanguine.prototype._jpgFile = function(src, target, quality, fn) {
    var _this = this;
    ...
    return easyimg.convert({
      ...
      return fn(null, target);
    });
  };

  sanguine.prototype._optimizeFile = function(src, target, colors, fn) {
    var _this = this;
